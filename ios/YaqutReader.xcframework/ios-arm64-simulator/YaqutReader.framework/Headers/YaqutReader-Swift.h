...
@class NSBundle;
/// Controller that displays and navigates through pages of a <code>PDFDocument</code>
...
@interface RRReaderView (SWIFT_EXTENSION(YaqutReader)) <UIGestureRecognizerDelegate>
/// Allows our page/cover tap gestures (<code>tapBG</code>, <code>coverTapBG</code>) to be recognized
/// alongside UIKit’s built-in text-view interactions instead of losing to them.
/// This delegate is only attached to those tap gestures, so it never affects
/// pinch, long-press, or any other recognizer. It guarantees <code>checkTapText</code>
/// runs on every tap — restoring link navigation and reliable bar toggling.
